...
class PasswordPolicyLowercase:
    ...
    def scan(self, option=None):
        ...
        if sb_utils.os.suse.pam.passwdqc_configured():
            ...
            msg2 = "If wish this module to configure 'lcredit', unconfigure " \
                   "pam_passwdqc."
        ...
        for pw_opt in cracklib_settings['password']:
            ...
            if pw_key == 'lcredit':
                msg = "PAM cracklib 'lcredit' option set to '%s' " % (pw_val)
                self.logger.info(self.module_name, msg)
                if int(pw_val) != -1:
                    msg = "PAM cracklib 'lcredit' option is not set to '-1'"
                    self.logger.notice(self.module_name, 'Scan Failed: ' + msg)
                    return 'Fail', msg
                else:
                    return 'Pass', ''
                    
        return 'Fail', 'lcredit parameter not set'
    ...
    def apply(self, option=None):
        ...
        if len(settings) != 0:
            lcredit = sb_utils.os.suse.pam.cracklib_get('lcredit') 
            action_record = "cracklib|%s" % str(lcredit)
            results = sb_utils.os.suse.pam.cracklib_set(option='lcredit', optValue='-1')
        ...
        # cracklib is DISABLED
            ...
            results = sb_utils.os.suse.pam.cracklib_set(option='lcredit', optValue='-1')
            if results != True:
                msg = "Unable to set pam_cracklib's lcredit parameter"
                self.logger.error(self.module_name, 'Apply Error: ' + msg)
                raise tcs_utils.ActionError('%s %s' % (self.module_name, msg))

        msg = "pam_cracklib's lcredit parameter set to '-1'"
...
#            raise tcs_utils.ActionError('%s %s' % (self.module_name, msg))
        ...
        if option == '':         
            msg = "'lcredit' has been unset"
            results = sb_utils.os.suse.pam.cracklib_unset(option='lcredit')
        else:
            msg = "'lcredit' has been restored to '%s'" % option
            results = sb_utils.os.suse.pam.cracklib_set(option='lcredit', optValue=option)
