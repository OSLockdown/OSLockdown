...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    <!-- =========================================================================
      ...
      Purpose: Apply Report XML to XHTML
     =========================================================================
-->
  <xsl:param name="report.title">Apply Report</xsl:param>
  <xsl:param name="css.file">/OSLockdown/css/apply-report.css</xsl:param>
      ...
      <body>
        ...
        <table class="sectionTable">
          ...
          <xsl:call-template name="profile.information">
            <xsl:with-param name="reportRoot" select="/ApplyReport"/>
          </xsl:call-template>

          <tr>
            <td rowspan="5" class="subSectionTitle" width="5%" style="text-align:right;">Results</td>
            <td class="infoName" rowspan="2"
                style="vertical-align: bottom; border-bottom: none; border-right: 1px solid black; text-align: right">
                                High Risk
            </td>
            <th class="statsHeader" style="text-align:center; border-top: none; color: red;">Errors</th>
            <th class="statsHeader" style="text-align:center; border-top: none; color: green">Applied</th>
          ...
          <tr>
            <td class="infoName"
                style="border-bottom: none; border-right: 1px solid black; text-align: right">
                                Medium Risk
            </td>
            <td class="statsCell" style="border-bottom: 1px dotted black;">
          ...
          <tr>
            <td class="infoName"
                 style="border-right: 1px solid black; border-bottom: none; text-align: right">
                                Low Risk
            </td>
            <td class="statsCell">
          ...
          <tr>
            <xsl:variable name="totalError" select="count(/ApplyReport/modules/module[@results='Error'])"/>
            <xsl:variable name="totalApplied" select="count(/ApplyReport/modules/module[@results='Applied'])"/>
            ...
            <td class="statsCell" style="border-top: 2px solid black; color: red; font-weight: bold">
              ...
              <xsl:text> (</xsl:text>
              <xsl:value-of select="round(($totalError div ($totalError + $totalApplied + $totalOther)) * 100)"/>
              <xsl:text>%)</xsl:text>
            </td>
            <td class="statsCell" style="border-left: 1px solid black; border-top: 2px solid black; color: green; font-weight: bold">
              ...
              <xsl:text> (</xsl:text>
              <xsl:value-of select="round(($totalApplied div ($totalError + $totalApplied + $totalOther)) * 100)"/>
              <xsl:text>%)</xsl:text>
            </td>
            <td class="statsCell" style="border-left: 1px solid black; border-top: 2px solid black; color: #467fc5">
              ...
              <xsl:text> (</xsl:text>
              <xsl:value-of select="round(($totalOther div ($totalError + $totalApplied + $totalOther)) * 100)"/>
              <xsl:text>%)</xsl:text>
            </td>
        ...
        <div style="padding-top: 10px; color: gray; width: 100%; text-align: left;">
          ...
          <!--
              ...
              ================================================================
          -->
        <xsl:if test="count(/ApplyReport/modules/module) != 0">
          <a name="modules"/>
        ...
        <xsl:if test="$footer.display != 'false'">
          <xsl:call-template name="footer">
            <xsl:with-param name="sbVersion" select="/ApplyReport/@sbVersion"/>
          </xsl:call-template>
        </xsl:if>
