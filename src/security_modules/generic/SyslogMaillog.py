...
#
#
# Ensures that /etc/syslog-ng/syslog-ng.conf has:
#
...
class SyslogMaillog:
    ...
    def __init__(self):
        self.module_name = "SyslogMaillog"
    ...
    def generate_changes(self, is_undo=False):
        ...
        if os.path.exists(logfile):
            ...
            search = [ '^filter\s+f_mailerr\s+\{\s+level\(err,\s+crit\)\s+and\s+facility\(mail\);\s+\}' ]
            replace = 'filter f_mailerr { level(err, crit) and facility(mail); };'
            if is_undo == True:
                ctm.append( {'search'       : search, 'regex': True  })
            else:
                ctm.append( {'search'       : search , 'replace_with' : replace })
                
            search = [ '^destination\s+mailerr\s+\{\s+file\(\"/var/log/mail.err\"\);\s+\}' ]
            replace =  'destination mailerr { file("/var/log/mail.err"); };'
            if is_undo == True:
                ctm.append( {'search'       : search, 'regex': True  })
            else:
                ctm.append( {'search'       : search , 'replace_with' : replace })
            ...
            search = [ '^log\s+\{\s+source\(src\);\s+filter\(f_mailerr\); destination\(mailerr\);\s+\}' ]
            replace = 'log { source(src); filter(f_mailerr); destination(mailerr); };'
        ...
        for cfgfile in ['/etc/rsyslog.conf' , '/etc/syslog.conf']:
            ...
            search =  [ '^mail.crit\s', '^mail.debug\s', '^mail.\*\s' ,'^\*.crit\s' ] 
            replace = 'mail.crit\t\t%s' % logfile 
            if is_undo == True:
