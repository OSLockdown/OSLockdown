...
class IPForward:
    """
    IPForward Security Module handles the guideline for IP Forwarding.
    """
    ...
    def __init__(self):
        ...
        self.__settings = {'net.ipv4.ip_forward': 0, 
        ...
        for param in  self.__settings.keys():
            if paramlist.has_key(param):
                ...
                if 'ipv6' in param.split('.') and ipv6_enabled == False:
                    msg = "Skipping - '%s' not found in %s" % (param, self.__target_file)
                    self.logger.info(self.module_name, msg)
                else:
        ...
        for xparam in self.__settings.keys():
            if paramlist.has_key(xparam):
                ...
                if 'ipv6' in xparam.split('.') and ipv6_enabled == False:
                    msg = "Skipping - '%s' not found in %s" % (xparam, self.__target_file)
                    self.logger.info(self.module_name, msg)
                    continue
