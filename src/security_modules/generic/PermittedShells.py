...
class PermittedShells:

    def __init__(self):

        self.module_name = "PermittedShells"
        self.__target_file = '/etc/shells'
    ...
    def checkContent(self, action, fileName, requiredChanges):
        ...
        if requiredChanges['content']:
            ...
            lexer.whitespace += ","
            allowedShells = tcs_utils.splitNaturally(requiredChanges['content'], wordAdditions="/-_")
            ...
            if linesLeft :
                messages.append("%s contains entries not on the approved list" % self.__target_file)
                for entry in linesLeft:
                    msg = "Found '%s' in '%s' " % ( entry, fileName)
                    self.logger.notice(self.module_name, "Scan Fail: " + msg)
        
            if linesMissing :
                messages.append("%s is missing required entries from the approved list" % self.__target_file)
    ...
    def scan(self, optionDict=None):
        ...
        requiredChanges['content'] = optionDict['allowedShells']           
        changes, messages = self.checkContent('scan', self.__target_file, requiredChanges)         
    ...
    def apply(self, optionDict=None):
        ...
        # set content for allow file
        requiredChanges['content'] = optionDict['allowedShells']           
        changes, messages = self.checkContent('apply', self.__target_file, requiredChanges)         
        allChanges.update(changes)
        allMessages.extend(messages)
    ...
    def undo(self, change_record=None):
        ...
        if not change_record.startswith('{'):
            ...
            for fileName, changesToUndo in change_record.items():
                changes, messages = self.checkContent('undo', fileName, changesToUndo)         
                allChanges.update(changes)
                allMessages.extend(messages)
