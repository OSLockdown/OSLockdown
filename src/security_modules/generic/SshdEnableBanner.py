...
class SshdEnableBanner(Ssh_Sshd_ConfigEditor.Ssh_Sshd_ConfigEditor):
    """
    SshdEnableBanner Security Module handles the guideline for 
    displaying a banner when connections are made to the server.
    """

    def __init__(self):
        Ssh_Sshd_ConfigEditor.Ssh_Sshd_ConfigEditor.__init__(self)
        self.module_name = "SshdEnableBanner"

        self.configfile = '/etc/ssh/sshd_config'
        self.settings = [ ['Banner', '/etc/issue.net',self._isEqualTo], 
                        ]
