...
#
# Notes:
#     In SUSE 11 the service is 'boot.sysstat' but in SUSE 10 it is 'sysstat'
#
#
#
##############################################################################
...
class EnableSysstat:
    ...
    def __init__(self):
        ...
        self._svcname = "boot.sysstat"
        try:
            suse_version =  float(sb_utils.os.info.getCpeName().split(':')[4])
            if suse_version >= 10.0 and suse_version < 11.0:
                self._svcname = "sysstat"
        except:
            # If there is an error, assume we are SUSE 11
            pass
