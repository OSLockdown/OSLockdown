...
test -d ./os && UTILSFILE=os/fedora/utilities.mk
test -d ../os && UTILSFILE=../os/fedora/utilities.mk
...
REQUIRED_RPMS="gcc swig make libxml2-devel openssl-devel python xmlto \
              python-devel rpm-build rpmdevtools redhat-lsb "
...
if [ ! -z "$UTILS" ]; then
    printf "\nVerifying paths specified in os/fedora/utilities.mk:\n"
    for i in $UTILS
    do
        printf "  %s" $i
...
DEVEL_RPMS="subversion splint valgrind strace"
for i in $DEVEL_RPMS
do
    devel_rpm $i
done
