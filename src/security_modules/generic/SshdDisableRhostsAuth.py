...
class SshdDisableRhostsAuth(Ssh_Sshd_ConfigEditor.Ssh_Sshd_ConfigEditor):
    """
    SshdDisableRhostsAuth Security Module handles the guideline for 
    disabling rhostsauthentication.
    """
    def __init__(self):
        Ssh_Sshd_ConfigEditor.Ssh_Sshd_ConfigEditor.__init__(self)
        self.module_name   = "SshdDisableRhostsAuth"

        self.configfile = '/etc/ssh/sshd_config'
        self.settings = [ ['RhostsAuthentication', 'no',self._isEqualTo], 
                        ]
