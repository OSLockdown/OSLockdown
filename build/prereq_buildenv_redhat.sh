...
#
# Build Platform: Red Hat (or clone)
#        Product: OS Lockdown
# 
...
test -d ./os && UTILSFILE=os/redhat/utilities.mk
test -d ../os && UTILSFILE=../os/redhat/utilities.mk
...
REQUIRED_RPMS="gcc gcc-c++ swig make libxml2-devel openssl-devel python \
              python-devel rpm-build redhat-lsb "
for i in $REQUIRED_RPMS
do
    required_rpm $i
done

printf "\nRPMs required to build Red Hat 4:\n"
ENT_RPMS="gcc4-c++"
for i in $ENT_RPMS
do
    devel_rpm $i
...
if [ ! -z "$UTILS" ]; then
    printf "\nVerifying paths specified in os/redhat/utilities.mk:\n"
    for i in $UTILS
    do
        printf "  %s" $i
...
DEVEL_RPMS="httpd mod_ssl subversion openjade doxygen splint valgrind strace"
for i in $DEVEL_RPMS
do
    devel_rpm $i
done
