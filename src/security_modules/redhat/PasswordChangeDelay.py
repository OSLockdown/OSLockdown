...
class PasswordChangeDelay:
    """
    PasswordChangeDelay Security Module handles the guideline for minimum
    allowed time between password changes.
    """
    ##########################################################################
    def __init__(self):
        self.module_name = "PasswordChangeDelay"
        self.__target_file = '/etc/login.defs'
        self.logger = TCSLogger.TCSLogger.getInstance()

    ##########################################################################
    def validate_input(self, optionDict):
        if not optionDict or not 'passwordMindays' in optionDict:
            return 1
        try:
            value = int(optionDict['passwordMindays'])
        except ValueError:
            return 1
        if value < 1:
            return 1
    ...
    def scan(self, optionDict):
        """
        Check the systems configured minimum password change time conforms to
        the guideline.
        """
        ...
        option = optionDict['passwordMindays']
        try:
        ...
        search_pattern = re.compile("^PASS_MIN_DAYS\s*\d")
  
        msg = "Checking %s for 'PASS_MIN_DAYS' setting" % (self.__target_file) 
        ...
        if value < int(option):
            reason = 'PASS_MIN_DAYS option is set to %d' % value
    ...
    def apply(self, optionDict):
        ...
        for line in lines:
            if search_pattern.search(line):
                ...
                new_line = 'PASS_MIN_DAYS\t' + str(option) + '\n'
        ...
        if not made_replacement:
            line_to_add = 'PASS_MIN_DAYS\t' + str(option) + '\n'
            out_obj.write('# Set the minimum days between password changes\n')
        ...
        msg = 'PASS_MIN_DAYS directive in /etc/login.defs set to %s' % \
              str(option)
    ...
    def undo(self, change_record):
        ...
        if not change_record or change_record == "" :
            msg = "Skipping undo - no change record or empty change record for module" 
            self.logger.error(self.module_name, 'Undo Error: ' + msg)
            raise tcs_utils.ActionError('%s %s' % (self.module_name, msg))
        ...
        msg = 'PASS_MIN_DAYS directive in /etc/login.defs reset to %s' % \
              str(old_value)
