...
class SshdDisableEmptyPasswords(Ssh_Sshd_ConfigEditor.Ssh_Sshd_ConfigEditor):
    """
    SshdDisableEmptyPasswords Security Module handles the guideline for not
    allowing the use of empty passwords.
    """

    def __init__(self):
        Ssh_Sshd_ConfigEditor.Ssh_Sshd_ConfigEditor.__init__(self)
        self.module_name    = "SshdDisableEmptyPasswords"

        self.configfile = '/etc/ssh/sshd_config'
        self.settings = [ ['PermitEmptyPasswords', 'no',self._isEqualTo], 
                        ]
