...
class SetXScreenSaverDefaults:
    ...
    def scan(self, optionDict=None):
        ...
        if not os.path.isfile('/usr/share/X11/app-defaults/XScreenSaver'):
            msg = 'Looking for /usr/share/X11/app-defaults/XScreenSaver'
        ...
        found = 0
        for conf_file in ['/usr/share/X11/app-defaults/XScreenSaver']:
            msg = "Checking %s for *timeout, *lockTimeout, and *lock settings." % conf_file
            ...
            for idx, line in enumerate(in_obj.readlines()):
                ...
                msg = "Line %d of %s, found: %s " % (idx+1, conf_file , line.strip())
                self.logger.info(self.module_name, msg)

                if param == '*lock' and parval != 'true':
                    msg = '*lock not set to true'
                    self.logger.notice(self.module_name, 'Scan Failed: ' + msg)
                    in_obj.close()
                    return 'Fail', msg

                if param == '*lockTimeout' and parval != '0:00:00':
                    msg = '*lockTimeout not set to 0:00:00'
                    self.logger.notice(self.module_name, "Scan Failed: " + msg)
                    in_obj.close()
                    return 'Fail', msg

                testval = '0:%02d:00' % int(option)
                if param == '*timeout' and parval != testval:
                    msg = '*timeout not set %s' % testval
                    self.logger.notice(self.module_name, "Scan Failed: " + msg)
                    in_obj.close()
                    return 'Fail', msg
    ...
    def apply(self, optionDict=None):
        ...
        for conf_file in ['/usr/share/X11/app-defaults/XScreenSaver']:
            ...
            try:
                ...
                sb_utils.SELinux.restoreSecurityContext(conf_file)
        ...
        if failure_flag == True:
            msg = 'Unable to set X screen saver options'
    ...
    def undo(self, change_record=None):
        ...
        msg = 'X Screen Saver options restored.'
