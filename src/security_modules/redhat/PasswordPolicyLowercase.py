...
class PasswordPolicyLowercase:
    def __init__(self):

        self.module_name = "PasswordPolicyLowercase"
    ...
    def scan(self, option=None):
        ...
        msg = 'Opening %s to check for lcredit' % self.__target_file
        ...
        for lineNr, line in enumerate(lines):
            ...
            if password.search(line):
                ...
                if lcredit.search(line):
                    msg = "Line %d of %s, pam_cracklib has 'lcredit' set" % (lineNr+1, self.__target_file)
                    self.logger.info(self.module_name, msg)
                    return 'Pass', ''
                else:
                    msg = "Line %d of %s, pam_cracklib is missing 'lcredit' option." % (lineNr+1, self.__target_file)
                    self.logger.info(self.module_name, msg)

        msg = 'lcredit option is not being used'
    ...
    def apply(self, option=None):
        ...
        for lineNr, line in enumerate(lines):
            ...
            if password.search(line):
                if lcredit.search(line):
                    ...
                    action_record = "added"
                    msg = "Added lcredit=-1 to pam_crackline on line %d of %s" % (lineNr+1, self.__target_file)
                    self.logger.info(self.module_name, 'Apply Performed: ' + msg)
                    out_obj.write("%s lcredit=-1\n" % (line.rstrip()))
            else:
                out_obj.write(line)
        ...
        if action_record == "added":
            msg = 'lcredit option set for pam_cracklib in %s' % (self.__target_file)
            self.logger.notice(self.module_name, 'Apply Performed: ' + msg)
            return 1, action_record
        else:
            msg = 'No changes required: lcredit option is already set for pam_cracklib in %s' % (self.__target_file)
    ...
    def undo(self, action_record=None):
        ...
        replacePat = re.compile(' lcredit=-\d+')

        for lineNr, line in enumerate(lines):
            line = line.lstrip(' ')
            if password.search(line):
                if lcredit.search(line):
                    msg = "Removed lcredit option from line %d of %s" % (lineNr+1, self.__target_file)
