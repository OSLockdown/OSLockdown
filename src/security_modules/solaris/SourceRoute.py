...
class SourceRoute:
    ...
    def __init__(self):
        ...
        self.__param       = {  'ip_forward_src_routed': 0, 
    ...
    def scan(self, option=None):
        ...
        if zonename != 'global':
            if not os.path.islink('/dev/ip') or not os.path.islink('/dev/tcp'):
                msg = "Non-global Solaris zone (%s): /dev/ip|tcp is unavailable" % (zonename)
        ...
        if failure_flag == True:
            msg = "Forwarding of source routed packets is NOT disabled"
            self.logger.notice(self.module_name, msg)
            return 'Fail', msg
        else:
            return 'Pass', ''
