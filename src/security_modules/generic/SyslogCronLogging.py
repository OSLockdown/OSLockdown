...
class SyslogCronLogging:
    ...
    def generate_changes(self, is_undo=False):
        ...
        if os.path.exists(logfile):
            ...
            search = [ '^filter\s+f_cron\s+\{\s+facility\(cron\);\s+\}' ]
            replace = 'filter f_cron { facility(cron); };'
            if is_undo == True:
                ctm.append( {'search'       : search, 'regex': True  })
            else:
                ctm.append( {'search'       : search , 'replace_with' : replace })
                
            search = [ '^destination\s+cron\s+\{\s+file\(\"/var/log/cron\"\);\s+\}' ]
            replace =  'destination cron { file("/var/log/cron"); };'
            if is_undo == True:
                ctm.append( {'search'       : search, 'regex': True  })
            else:
                ctm.append( {'search'       : search , 'replace_with' : replace })
            ...
            search = [ '^log\s+\{\s+source\(src\);\s+filter\(f_cron\); destination\(cron\);\s+\}' ]
            replace = 'log { source(src); filter(f_cron); destination(cron); };'
