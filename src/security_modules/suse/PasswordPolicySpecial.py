...
class PasswordPolicySpecial:
    ...
    def scan(self, option=None):
        ...
        if sb_utils.os.suse.pam.passwdqc_configured():
            ...
            msg2 = "If wish this module to configure 'ocredit', unconfigure " \
                   "pam_passwdqc."
        ...
        for pw_opt in cracklib_settings['password']:
            ...
            if pw_key == 'ocredit':
                msg = "PAM cracklib 'ocredit' option set to '%s' " % (pw_val)
                self.logger.info(self.module_name, msg)
                if int(pw_val) != -2:
                    msg = "PAM cracklib 'ocredit' option is not set to '-2'"
                    self.logger.notice(self.module_name, 'Scan Failed: ' + msg)
                    return 'Fail', msg
                else:
                    return 'Pass', ''
                    
        return 'Fail', 'ocredit parameter not set'
    ...
    def apply(self, option=None):
        ...
        if len(settings) != 0:
            ocredit = sb_utils.os.suse.pam.cracklib_get('ocredit') 
            action_record = "cracklib|%s" % str(ocredit)
            results = sb_utils.os.suse.pam.cracklib_set(option='ocredit', optValue='-2')
        ...
        # cracklib is DISABLED
            ...
            results = sb_utils.os.suse.pam.cracklib_set(option='ocredit', optValue='-2')
            if results != True:
                msg = "Unable to set pam_cracklib's ocredit parameter"
                self.logger.error(self.module_name, 'Apply Error: ' + msg)
                raise tcs_utils.ActionError('%s %s' % (self.module_name, msg))

        msg = "pam_cracklib's ocredit parameter set to '-2'"
...
#            raise tcs_utils.ActionError('%s %s' % (self.module_name, msg))
        ...
        if option == '':         
            msg = "'ocredit' has been unset"
            results = sb_utils.os.suse.pam.cracklib_unset(option='ocredit')
        else:
            msg = "'ocredit' has been restored to '%s'" % option
            results = sb_utils.os.suse.pam.cracklib_set(option='ocredit', optValue=option)
