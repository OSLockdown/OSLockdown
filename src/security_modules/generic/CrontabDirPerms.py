...
class CrontabDirPerms:
    """
    """
    ##########################################################################
    def __init__(self):

        self.module_name = "CrontabDirPerms"
    ...
    def checkItems(self, optionDict, action):
        ...
        # We are explicitly given the directory names to use, so complain if no entry exists.
        
        fileList = []
        for dirEntry in sb_utils.file.fileperms.splitStringIntoFiles(optionDict['crontabDirList']):
...
if __name__ == '__main__':
    ...
    test = CrontabDirPerms()
    optDict = {'sysCrontabDirs': 'var/spool/cron /etc/cron.d /etc/cron.hourly /etc/cron.daily /etc/cron.weekly /etc/cron.monthly',
               'allowedUnames': 'root',
