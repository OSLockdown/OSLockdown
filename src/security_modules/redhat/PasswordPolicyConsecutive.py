...
class PasswordPolicyConsecutive:
    def __init__(self):

        self.module_name = "PasswordPolicyConsecutive"
    ...
    def validate_input(self, optionDict):
        if not optionDict or not 'passwordConsecutiveChars' in optionDict:
            return 1
        try:
            value = int(optionDict['passwordConsecutiveChars'])
        except ValueError:
            return 1
        if value < 1:
            return 1
    ...
    def scan(self, optionDict=None):
        ...
        msg = 'Checking pam_cracklib maxrepeat setting in %s' % (self.__target_file)
        self.logger.info(self.module_name, msg)

        lines = in_obj.readlines()
        in_obj.close()

        maxrepeat = re.compile('maxrepeat')
        ...
        for line in lines:
            ...
            if password.search(line):
                if maxrepeat.search(line):
                    matched_line = line
        if matched_line:
            tokens = matched_line.split()
            for token in tokens:
                if token.startswith('maxrepeat'):
                    value_tokens = token.split('=')
                    value = int(value_tokens[1])

        if value >= int(option):
            msg = 'pam_cracklib maxrepeat option is set to %d which is greater than or equal to %d' % (value, int(option))
            self.logger.info(self.module_name, msg)
        else:
            if not matched_line:
                msg = 'pam_cracklib maxrepeat option not set'
            else:
                msg = 'pam_cracklib maxrepeat option is set to %d instead of %d' % (value, int(option))
    ...
    def apply(self, optionDict=None):
            ...
            if password.search(line) :
                ...
                if match_obj:
                    ...
                    if int(option) > int(old_value):
                        new_line = re.sub(old_string, 'maxrepeat=%d' % int(option), line)
                        action_record.append('pam_maxrepeat=%d' % int(old_value))
                    else:
                        # no substitution needed
                        new_line = line
                else:
                    new_line = line.rstrip('\n') + ' maxrepeat=%d\n' % int(option)
                    action_record.append('pam_maxrepeat=unset')
        ...
        msg = 'maxrepeat option set for pam_cracklib in system-auth'
                ...
                for line in lines:
                    ...
                    if password.search(line):
                        ...
                        if match_obj:
                            ...
                            old_value = tokens[1]
                            if pairValue != 'unset':
                                new_line = re.sub(old_string, 'maxrepeat=%d' % int(pairValue), line)
                                msg = "pam_cracklib's 'maxrepeat' option restored to %d" % (int(pairValue))
                            else:
                                # no substitution needed
...
if __name__ == '__main__':
    ...
    dict = {'passwordConsecutiveChars' : 4}
    
    test = PasswordPolicyConsecutive()
