...
class LockInactiveAccount:

    def __init__(self):
        self.module_name = "LockInactiveAccount"

        self._config_file = '/etc/default/useradd'
        self.keyname = 'INACTIVE'
        self.delim = ['=']
    ...
    def validate_input(self, optionDict):
        if not optionDict or not 'daysInactive' in optionDict:
            return 1
        try:
            value = int(optionDict['daysInactive'])
        except ValueError:
            return 1
        if value < 1:
            return 1
