...
class ArpCleanupInterval_IRE:
    ##########################################################################
    def __init__(self):

        self.module_name = "ArpCleanupInterval_IRE"

        self.logger = TCSLogger.TCSLogger.getInstance()

        self.__ndd_file = '/etc/default/ndd'
        self.__desc     = 'ARP IRE_CACHE Scan Interval'
        self.__param    = {  'ip_ire_arp_interval': 60000 }
    ...
    def validate_input(self, optionDict=None):
        ...
        is_valid = False
        minValue = self.__param['ip_ire_arp_interval']
        try:
            optionValue = int(optionDict['arpCleanupIntervalIRE'])
        ...
        if not is_valid:
            msg = "Parameter is invalid/missing, must be a integer greater than %d, forcing to %d" % (minValue, minValue)
            self.logger.error(self.module_name, msg)
            optionValue = minValue
        
        self.__param['ip_ire_arp_interval'] = optionValue
